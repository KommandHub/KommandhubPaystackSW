...
readonly class TransactionVerificationProcessor implements TransactionVerificationProcessorInterface
{
    ...
    public function verify(
        ...
        $this->assertTransactionCurrency($data, $transaction);
    ...
    /**
     * Guards against a swapped callback reference that points at a transaction
     * in a different currency whose minor-unit integer collides with the order.
     *
     * @param array<string, mixed> $data
     */
    private function assertTransactionCurrency(
        array $data,
        OrderTransactionEntity $transaction
    ): void {
        $order = $transaction->getOrder();
        ...
        $expected = strtoupper($order->getCurrency()->getIsoCode());
        ...
        if ($received !== $expected) {
            ...
            $this->fail(
                $transaction->getId(),
                'Payment currency mismatch detected.'
            );
        }
    }
